...
- **Un conjunto cerrado de scripts revisados**, instalados una vez en `/opt/vertiguard-mcp` en el VPS, cada uno lo bastante corto como para leerlo en un minuto (ver [`scripts/`](scripts/)).
...
```bash
scp -r scripts/ tuusuario@tu-vps:/tmp/vertiguard-mcp-scripts
ssh tuusuario@tu-vps
cd /tmp/vertiguard-mcp-scripts && sudo ./install.sh
```

Esto crea un usuario del sistema dedicado `vertiguard-mcp`, instala los scripts en `/opt/vertiguard-mcp`, escribe una regla `sudoers` que limita ese usuario a exactamente esos scripts, y genera un par de claves SSH dedicado. Al terminar imprime los siguientes pasos, incluido dónde copiar la clave privada.

Edita `/opt/vertiguard-mcp/allowed_services.conf` en el VPS para listar los servicios que realmente quieres poder reiniciar (uno por línea — viene con `nginx` y `mariadb` como ejemplo).
  ...
  "mcpServers": {
    "vertiguard": {
      ...
      "env": {
        "VPS_HOST": "tu-ip-o-dominio-del-vps",
        "VPS_USER": "vertiguard-mcp",
        "VPS_SSH_KEY_PATH": "/ruta/absoluta/a/vertiguard-mcp-key",
        "SCRIPTS_DIR": "/opt/vertiguard-mcp",
        "ALLOWED_SERVICES": "nginx,mariadb"
      }
    }
...
Añade un script nuevo en `scripts/`, súmalo a la regla `sudoers` volviendo a ejecutar `install.sh`, y registra la herramienta correspondiente con `@mcp.tool()` en `src/vertiguard_mcp/server.py`. Los pull requests con nuevas comprobaciones de solo lectura son especialmente bienvenidos.
